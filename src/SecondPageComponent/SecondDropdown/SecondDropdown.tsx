...
const SecondDropdown: React.FC<DropdownProps> = ({ allJokes, setAllJokes, handleToggle }) => {
  ...
  return (
    <>
      <div className='dropdown absolute top-0 left-[98px] bg-[#315d6e] outline-1 outline outline-[#46a8cc] px-4 z-10 rounded-b-lg'>
        <div>
    <h3 className='py-2 border-b-[1px] text-white text-sm font-semibold  border-b-[#46a8cc]'>Filters</h3>
        ...
        <div className='md:grid grid-cols-2 gap-x-4 '>
          ...
          {filterList.map((eachDropdown, index) => {
            return (
              <div key={index}>
                <h4 className='text-[#e7ecf2] text-sm  font-semibold py-2 flex items-center pl-2 cursor-pointer ' onClick={()=>handleListToggle(eachDropdown.name)}><IoIosArrowDown /> &nbsp; {eachDropdown.name}</h4>
                {!showListItem[eachDropdown.name] ?(
                <div className='flex items-center'>
                  <div className="input-with-icon relative w-[280px] bg-[#33738C]  py-2 text-sm border-none rounded-md hover:outline-1 focus:outline focus:outline-[#46a8cc] ">
                    <FaSearch className='search-icon !text-[#e7ecf2]' />
                    <input type="text" placeholder="Searh by some words" id={eachDropdown.name} className='bg-transparent pl-10 outline-none text-[#d5dde5] placeholder:text-[#e7ecf2]' onChange={(e)=>setSearchCategory({[eachDropdown.name]:e.target.value})} onBlur={(e)=>setSearchCategory({[eachDropdown.name]:""})} value={searchCategory[eachDropdown.name]}/>
                ...
                {!showListItem[eachDropdown.name] ?
                  ...
                  <ul className='filter-list space-y-[7px] text-[#E5DBD5] py-3 pl-3 '>

                    {
                      eachDropdown.name===Object.keys(searchCategory)[0]? eachDropdown.items.filter((eachItem)=>{
                        ...
                        return (
                          ...
                          <li key={index}><input type='checkbox' id={eachListItem}  checked={checkedItems[eachListItem]} onChange={handleCheckboxChange} /> <label htmlFor={eachListItem} className='text-sm'>{eachListItem}</label></li>
